...
describe('appsync-client', () => {
  ...
  it('publishRoutesGenerated sends correct payload', async () => {
    ...
    const body = JSON.parse(opts.body);
    expect(body).toEqual({
      query: `mutation PublishRoutesGenerated($jobId: ID!, $routes: [RouteInput]!, $correlationId: ID, $version: Int!) {\n  publishRoutesGenerated(jobId: $jobId, routes: $routes, correlationId: $correlationId, version: $version)\n}`,
      variables: {
        jobId: 'job-1',
        routes: [
          {
